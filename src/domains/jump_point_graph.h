#ifndef WARTHOG_GRAPH_JUMP_POINT_GRAPH_H
#define WARTHOG_GRAPH_JUMP_POINT_GRAPH_H

// domains/jump_point_graph.h
//
// A graph in which every node is a jump point and every edge
// is the result of a forward jump in a fixed direction 
//
// An insertion operation is used to handle cases when the 
// start or target location is not a jump point.
//
// ***********************************************************************
// *************    INSERTION OF THE GOAL DOES NOT WORK      *************
// *************               DO NOT USE                    ************* 
// ***********************************************************************
// 
// @author: dharabor
...
class jump_point_graph
{
    ...
    struct label_index
    {
        ...
        clear()
        {
            uint32_t num_labels = warthog::graph::jump_point_graph::NUM_LABELS;
            for(uint32_t i = 0; i < num_labels; i++)
                head_[i] = 0;
        }

        warthog::graph::ECAP_T head_[warthog::graph::jump_point_graph::NUM_LABELS];
    };

    public:
        jump_point_graph( warthog::gridmap* map );
                

        virtual ~jump_point_graph();
        ...
        // (up to warthog::graph::jump_point_graph::NUM_LABELS)
    ...
    private:
        bool verbose_;
        warthog::gridmap* corner_map_;
        warthog::gridmap* gm_;
        warthog::graph::planar_graph* g_;
        warthog::jps::online_jump_point_locator2* jpl_;
        
        // no copy ctor
        jump_point_graph(warthog::graph::jump_point_graph& other) : S_DUMMY_ID(0), T_DUMMY_ID(0) { }
