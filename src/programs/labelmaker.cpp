...
//{
//    std::string chd_filename = cfg.get_param_value("input");
//    if( chd_filename == "")
//    {
...
//
//    // load up the graph
//    std::shared_ptr<warthog::ch::ch_data> chd_sptr(
//        warthog::ch::load_ch_data(chd_filename, warthog::ch::UP_DOWN));
//    warthog::ch::ch_data* chd = chd_sptr.get();
//    warthog::graph::xy_graph* g = chd->g_;
//
//    // define the preprocessing workload 
//    warthog::util::workload_manager workload(g->get_num_nodes());
//
//    // users can limit the preprocessing to a specified level range 
//    uint32_t first_level = 0;
//    uint32_t last_level = (uint32_t)g->get_num_nodes();
...
//        {
//            first_level = (uint32_t)strtol(first.c_str(), 0, 10);
//        }
//        if(strtol(last.c_str(), 0, 10) != 0)
//        {
//            last_level = (uint32_t)strtol(last.c_str(), 0, 10);
//        }
//    }
//    if(first_level > last_level)
//    {
//        std::cerr << "err; invalid level range: --type [first] [last].\n"
//        return;
//    }
//
//    // we only target nodes in a specified level range
//    for(uint32_t i = first_level; i < last_level; i++)
//    {
//        workload.set_flag(chd->level_->at(i), true);
//    }
//
//    // gogogo
...
//
//    // save the result
//    std::string outfile(chd_filename);
