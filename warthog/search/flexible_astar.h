...
template <class H, class E, class L>
class flexible_astar 
{
	public:
		flexible_astar(H* heuristic, E* expander, L* logger)
		...
		get_length(uint32_t startid, uint32_t goalid)
		{
			...
			if(verbose_)
			{
				...
				/*while(!path.empty())
				{
					...
					uint32_t x, y;
					y = (n->get_id() / expander_->mapwidth());					<----- Final path here, TODO
	...
	private:
		...
		search(uint32_t startid, uint32_t goalid)
		{
			...
			if(verbose_)
			{
				logger_->log(PRELUDE, (warthog::search_node*)NULL);
				logger_->log(SOURCE, expander_->generate(startid));
				logger_->log(DESTINATION, expander_->generate(goalid));
			}
			#endif
...
template <class H, class E, class L>
uint32_t warthog::flexible_astar<H, E, L>::searchid_ = 0;
